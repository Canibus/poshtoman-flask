...
from ..models.WebAdminModel import WebAdminModel
...
class AuthAdmin():
    ...
    def generate_token(admin_id):
        try:
            ...
            return Response(
                mimetype="application/json",
                response=json.dumps({'error': 'error in generating admin token'}),
                status=400
            )
    ...
    def auth_required(func):
        ...
        def decorated_auth(*args, **kwargs):
            ...
            token = request.headers.get('api-token')
            data = AuthAdmin.decode_token(token)
            if data['error']:
                return Response(
                mimetype="application/json",
                response=json.dumps(data['error']),
            ...
            check_admin = WebAdminModel.get_one_admin(admin_id)
            if not check_admin:
                return Response(
                mimetype="application/json",
                response=json.dumps({'error': 'user does not exist, invalid token'}),
